...
class Model(nn.Module):
    ...
    def add_pos_encoding(self, features, pos_embed):
        """Helper to add positional encoding with interpolation"""
        pos = pos_embed.to(device=features.device, dtype=features.dtype)
        if pos.shape[-2:] != features.shape[-2:]:
            pos = F.interpolate(pos, size=features.shape[-2:], 
                              mode='bilinear', align_corners=False)
        self._validate_progressive_chain()
        return features + pos
    def _validate_progressive_chain(self):
        """Validate that progressive modules are properly ordered"""
        progressive_layers = []
        bcam_layers = []
        
        for i, module in enumerate(self.model):
            if isinstance(module, BCAM_Progressive):
                progressive_layers.append(i)
            elif isinstance(module, BCAM):
                bcam_layers.append(i)
        
        
        # Ensure BCAM comes before BCAM_Progressive
        if bcam_layers and progressive_layers:
            if max(bcam_layers) >= min(progressive_layers):
                print("WARNING: BCAM should come before BCAM_Progressive")

    def forward_once(self, x, x2, profile=False):
        """
        Forward pass with progressive chaining orchestration
        """
        y, dt = [], []
        ...
        for m in self.model:
            ...
            if isinstance(m, BCAM_Progressive):
                ...
                if isinstance(x, tuple) and len(x) == 3:
                    ...
                    fused_contexts[scale] = fused_out  # Store fused for next progressive layer
                    x = fused_out  # Use fused for next layer
                elif isinstance(x, tuple) and len(x) == 2:
                    # Fallback for 2-tuple (backward compatibility)
                ...
                if not isinstance(x, (tuple, list)) or len(x) != 2:
                    raise RuntimeError(f"UCAM at layer {i} expects tuple input (rgb, thermal), got {type(x)}")
                x = m(x, pos_rgb=None, pos_thermal=None)
                ...
                if isinstance(x, tuple) and len(x) == 3:
                    ...
                    try:
                        ...
                        # Non-standard UCAM, skip storing
                        x = fused_out
                elif isinstance(x, tuple) and len(x) == 2:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
            ...
            print(f"DEBUG: ChannelAttention module with f={f}")
            if isinstance(f, list):
                c2 = ch[f[0]]  # Output channels same as feature input
                feature_channels, context_channels = args[0], args[1]
                m_ = ChannelAttention(feature_channels, context_channels)
            else:
                raise ValueError("ChannelAttention requires list input [features, context]")
        elif m is Detect:
